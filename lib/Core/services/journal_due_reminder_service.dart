...
class JournalDueReminderService {
  ...
  static final JournalDueReminderService instance =
      JournalDueReminderService._();
  static final DateTime _serviceStartDate = DateTime.now();
  ...
  void start() {
    ...
    (_lifecycleObserver as _JournalLifecycleObserver).onResumed = () async {
      ...
      // Delay scan to allow Notification Taps to process first
  ...
  Future<void> _handleDueSnapshot(
    ...
    final dueDocs =
        snapshot.docs.where((doc) {
          ...
          // Grace period: allow reminders due up to 3 seconds in the future to trigger now
          if (reminderAt.isAfter(now.add(const Duration(seconds: 3)))) {
            if (closestFutureTime == null ||
                reminderAt.isBefore(closestFutureTime!)) {
  ...
  void _schedulePreciseTimer(DateTime time) {
    _nextReminderTimer?.cancel();
    // Schedule for the exact time, but allow a small buffer for processing
    final duration = time.difference(DateTime.now());
    
    _nextReminderTimer = Timer(duration.isNegative ? Duration.zero : duration, () {
      scanDueRemindersNow();
    });
  }

  Future<void> _processDueReminder(
    String docId,
    ...
    // Strict startup spam protection: If the app just opened (within 10 seconds),
    // aggressively clean old reminders WITHOUT claiming them, so they don't pop up 
    // randomly, but push notification taps can still confidently bypass and show!
    final bool isStartup = DateTime.now().difference(_serviceStartDate).inSeconds < 10;
    ...
    // Now safely add to handled items 
  ...
  Future<void> _consumeAcknowledgedReminder(
    ...
    try {
      ...
      if (showAcknowledgedPopup) {
        // ALWAYS forcefully clear dialog lock for explicitly tapped notifications
        final ctx = await _waitForNavigatorContext();
        if (ctx != null) {
          _dialogOpen = true;
          try {
            ...
            debugPrint('Failed to show dialog on TAP: \$e');
          } finally {
            _dialogOpen = false;
          }
        }
  ...
  Future<void> reconcilePendingAcknowledgedReminders() async {
    final pending =
        await NotificationService.getPendingAcknowledgedReminderDocIds();
