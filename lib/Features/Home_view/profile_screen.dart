...
class _ProfileViewBodyState extends State<ProfileViewBody> {
  ...
  void _showLanguageSelector() {
    ...
    showModalBottomSheet(
      context: context,
      backgroundColor: isDark ? kDarkSurfaceColor : kLightSurfaceColor,
      ...
      builder: (_) => SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(context.t('Select Language', 'اختر اللغة'), 
                style: FontStyles.getRoboto18(context).copyWith(fontWeight: FontWeight.bold)),
              SizedBox(height: 10.h),
              ListTile(
                leading: const Text('🇺🇸', style: TextStyle(fontSize: 24)),
                title: Text('English', style: TextStyle(color: colorScheme.onSurface)),
                onTap: () { 
              ...
              ListTile(
                leading: const Text('🇪🇬', style: TextStyle(fontSize: 24)),
                title: Text('العربية', style: TextStyle(color: colorScheme.onSurface)),
                onTap: () { 
  ...
  void _populateFromUser(UserModel user) {
    ...
    if (rawData.isNotEmpty) {
      ...
      if (cleanCode.length > 3) cleanCode = '';
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<ProfileCubit, ProfileState>(
      ...
      builder: (context, state) {
        if (state is ProfileLoading) return const Scaffold(body: Center(child: CircularProgressIndicator()));

        final isDark = Theme.of(context).brightness == Brightness.dark;
        final theme = Theme.of(context);
        final colorScheme = theme.colorScheme;
        
        final String currentGender = genderSelection;
                  ...
                  if (!widget.onboarding && state is ProfileLoaded)
                    IconButton(
                      icon: Icon(
                        ...
                        color: _editing ? Colors.redAccent : colorScheme.onSurface,
                      ),
                    ...
                    children: [
                        ...
                        Column(
                          children: [
                            ...
                            Text(
                              ...
                              style: FontStyles.getRoboto14(context).copyWith(color: colorScheme.onSurface.withValues(alpha: 0.7)),
                            ),
                      ...
                      if (showPasswordLink) ...[
                        ...
                        Text(
                          ...
                          style: FontStyles.getRoboto12(context).copyWith(color: colorScheme.onSurface.withValues(alpha: 0.7)),
                        ),
                        SizedBox(height: 12.h),
                        _buildPasswordField(
                        ...
                        SizedBox(
                          ...
                          child: OutlinedButton.icon(
                            ...
                            label: Text(
                              ...
                              style: FontStyles.getRoboto16(context).copyWith(color: kAccentColor),
                            ),
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            ...
                            child: state is ProfileUpdating
                              ...
                              : Text(context.t("Save Changes", "حفظ التعديلات"), style: FontStyles.getRoboto18(context).copyWith(color: Colors.white, fontWeight: FontWeight.bold)),
                          ),
                        ),
                        if (!widget.onboarding) ...[
                          SizedBox(height: 12.h),
                          SizedBox(
                            ...
                            child: OutlinedButton(
                              ...
                              child: Text(context.t("Cancel", "إلغاء"), style: FontStyles.getRoboto18(context).copyWith(color: Colors.grey, fontWeight: FontWeight.bold)),
                            ),
                          ),
                        ],
                      ],
  ...
  Widget _buildPasswordField(String label, TextEditingController controller, {required bool obscure}) {
    final colorScheme = Theme.of(context).colorScheme;
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(label, style: FontStyles.getRoboto14(context).copyWith(color: colorScheme.onSurface.withValues(alpha: 0.7))),
          SizedBox(height: 8.h),
          TextField(
            ...
            decoration: InputDecoration(
              ...
              focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16.r), borderSide: const BorderSide(color: kAccentColor, width: 1)),
            ),
          ),
        ],
      ),
    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
              color: (isDark ? Colors.black : kAccentColor).withValues(alpha: isDark ? 0.2 : 0.08),
              blurRadius: 10,
        ...
        child: Row(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: kAccentColor.withValues(alpha: 0.12), shape: BoxShape.circle),
            ...
            Expanded(
              child: Column(
                ...
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Text(label, style: FontStyles.getRoboto12(context).copyWith(color: colorScheme.onSurface.withValues(alpha: 0.54), fontWeight: FontWeight.w500)),
                  SizedBox(height: 4.h),
                  Directionality(
                    textDirection: forceLtr ? TextDirection.ltr : (context.isArabic ? TextDirection.rtl : TextDirection.ltr),
                    child: Text(
                      value.isEmpty ? context.t('Not specified', 'غير محدد') : value,
                      style: FontStyles.getRoboto16(context).copyWith(fontWeight: FontWeight.w600),
  ...
  Widget _buildDiseasesChips() {
    ...
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(context.t("Chronic Diseases", "الأمراض المزمنة"), style: FontStyles.getRoboto14(context).copyWith(color: colorScheme.onSurface.withValues(alpha: 0.7))),
          SizedBox(height: 8.h),
          Wrap(
            spacing: 8.w,
            runSpacing: 8.h,
            children: items.map((disease) {
              return Chip(
                label: Text(_translateDisease(disease, context), style: FontStyles.getRoboto14(context)),
  ...
  Widget _buildField(String label, TextEditingController controller, IconData icon, {bool isNumber = false, bool isMultiline = false, bool isPhone = false, bool readOnly = false}) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    if (readOnly) return _buildReadonlyCard(label, controller.text, icon, isMultiline: isMultiline, forceLtr: isPhone);
    final colorScheme = Theme.of(context).colorScheme;
    return Padding(
        ...
        children: [
          ...
          Directionality(
            ...
            child: TextField(
              ...
              style: TextStyle(color: colorScheme.onSurface.withValues(alpha: readOnly ? 0.58 : 1)),
              decoration: InputDecoration(
                prefixIcon: Icon(icon, color: kAccentColor.withValues(alpha: readOnly ? 0.4 : 1), size: 20.sp),
                filled: true,
                fillColor: colorScheme.surface,
                contentPadding: EdgeInsets.symmetric(horizontal: 16.w, vertical: 16.h),
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(16.r), borderSide: const BorderSide(color: Colors.transparent)),
                enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16.r), borderSide: BorderSide(color: isDark ? Colors.transparent : colorScheme.onSurface.withValues(alpha: 0.1))),
                focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16.r), borderSide: BorderSide(color: kAccentColor, width: 1.5)),
              ),
            ),
          ),
        ],
  ...
  Widget _buildCountryField({bool readOnly = false}) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    if (readOnly) return _buildReadonlyCard(context.t("Country", "الدولة"), countryController.text, Icons.public, isCountry: true);
    final colorScheme = Theme.of(context).colorScheme;
    return Padding(
      padding: EdgeInsets.only(bottom: 20.h),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(context.t("Country", "الدولة"), style: FontStyles.getRoboto14(context).copyWith(color: colorScheme.onSurface.withValues(alpha: 0.7))),
          SizedBox(height: 8.h),
          InkWell(
            borderRadius: BorderRadius.circular(16.r),
            onTap: () {
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: colorScheme.surface,
                borderRadius: BorderRadius.circular(16.r),
                border: Border.all(color: isDark ? Colors.transparent : colorScheme.onSurface.withValues(alpha: 0.1)),
              ),
              child: Row(
                children: [
                  Icon(Icons.public, color: kAccentColor, size: 20.sp),
                  SizedBox(width: 12.w),
                  Expanded(child: Text(countryController.text.isEmpty ? context.t("Select Country", "اختر الدولة") : countryController.text, style: TextStyle(color: colorScheme.onSurface))),
                  Icon(Icons.arrow_drop_down, color: colorScheme.onSurface.withValues(alpha: 0.7)),
                ],
              ),
            ),
          ),
  ...
  Widget _buildGenderRadioButtons({bool readOnly = false}) {
    if (readOnly) return _buildReadonlyCard(context.t("Gender", "الجنس"), genderSelection.toLowerCase() == "male" ? context.t("Male", "ذكر") : context.t("Female", "أنثى"), genderSelection.toLowerCase() == "female" ? Icons.female : Icons.male);
    return Row(
      children: [
  ...
  Widget _buildGenderOption(String value, String label) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSelected ? kAccentColor.withValues(alpha: 0.2) : colorScheme.surface,
          borderRadius: BorderRadius.circular(12.r),
          border: Border.all(color: isSelected ? kAccentColor : Colors.transparent),
        ),
        child: Row(
          children: [
            Icon(value.toLowerCase() == "male" ? Icons.male : Icons.female, color: isSelected ? kAccentColor : colorScheme.onSurface.withValues(alpha: 0.7), size: 20.sp),
            SizedBox(width: 8.w),
            Text(label, style: TextStyle(color: isSelected ? colorScheme.onSurface : colorScheme.onSurface.withValues(alpha: 0.7), fontWeight: isSelected ? FontWeight.bold : FontWeight.normal)),
          ],
        ),
      ),
    );
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Text(context.t('Legal & information', 'المعلومات والقوانين'), style: FontStyles.getRoboto16(context).copyWith(color: kAccentColor, fontWeight: FontWeight.w700)),
          ),
          ListTile(
            dense: true,
            leading: Icon(Icons.privacy_tip_outlined, color: kAccentColor, size: 22.sp),
            title: Text(context.t('Privacy Policy', 'سياسة الخصوصية'), style: FontStyles.getRoboto14(context)),
            trailing: Icon(Icons.chevron_right_rounded, color: colorScheme.onSurface.withValues(alpha: 0.54), size: 22.sp),
            onTap: () => context.push(AppRouter.kPrivacyPolicyView),
          ),
          ListTile(
            dense: true,
            leading: Icon(Icons.article_outlined, color: kAccentColor, size: 22.sp),
            title: Text(context.t('Terms of Service', 'شروط الخدمة'), style: FontStyles.getRoboto14(context)),
            trailing: Icon(Icons.chevron_right_rounded, color: colorScheme.onSurface.withValues(alpha: 0.54), size: 22.sp),
          ...
          ListTile(
            ...
            title: Text(context.t('Monitoring guide', 'دليل المراقبة'), style: FontStyles.getRoboto14(context)),
            trailing: Icon(Icons.menu_book_outlined, color: colorScheme.onSurface.withValues(alpha: 0.54), size: 20.sp),
            onTap: () => showMonitoringGuideManual(context),
          ),
          ListTile(
            dense: true,
            leading: Icon(Icons.medical_information_outlined, color: kAccentColor, size: 22.sp),
            title: Text(context.t('Medical disclaimer', 'إخلاء المسؤولية الطبية'), style: FontStyles.getRoboto14(context)),
            trailing: Icon(Icons.chevron_right_rounded, color: colorScheme.onSurface.withValues(alpha: 0.54), size: 22.sp),
            onTap: () => context.push(AppRouter.kMedicalDisclaimerView, extra: true),
          ),
        ],
      ),
    );
  ...
  Widget _buildDefaultAvatar(String gender) {
    ...
    String avatarUrl = isFemale ? "https://cdn-icons-png.flaticon.com/512/3135/3135823.png" : "https://cdn-icons-png.flaticon.com/512/3135/3135715.png";
    return Image.network(avatarUrl, fit: BoxFit.cover, errorBuilder: (context, error, stackTrace) => Icon(Icons.person, color: Theme.of(context).colorScheme.onSurface, size: 40));
  }
}
...
class _DiseasesEditorWidgetState extends State<_DiseasesEditorWidget> {
  ...
  void _parseController() {
    ...
    if (text.isEmpty) { diseases = []; } else { diseases = text.split(RegExp(r'[,،\n]')).map((e) => e.trim()).where((e) => e.isNotEmpty).toList(); }
  }
  void _updateController() { widget.controller.text = diseases.join(', '); }
  void _addDisease(String val) {
    final trimmed = val.trim();
    if (trimmed.isNotEmpty && !diseases.contains(trimmed)) { setState(() { diseases.add(trimmed); }); _updateController(); }
    _inputCtrl.clear();
    _focusNode.requestFocus();
  }
  void _removeDisease(String val) { setState(() { diseases.remove(val); }); _updateController(); }
  @override
  Widget build(BuildContext context) {
    final colorScheme = Theme.of(context).colorScheme;
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(widget.labelTitle, style: FontStyles.getRoboto14(context).copyWith(color: colorScheme.onSurface.withValues(alpha: 0.7))),
          SizedBox(height: 8.h),
          Container(
            padding: EdgeInsets.all(12.r),
            decoration: BoxDecoration(color: colorScheme.surface, borderRadius: BorderRadius.circular(16.r)),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: [
                if (diseases.isNotEmpty)
                  Wrap(
                    ...
                    children: diseases.map((d) {
                      return Chip(
                        label: Text(d, style: TextStyle(color: colorScheme.onSurface, fontSize: 13.sp)),
                        backgroundColor: kAccentColor.withValues(alpha: 0.2),
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    hintStyle: TextStyle(color: colorScheme.onSurface.withValues(alpha: 0.3), fontSize: 14.sp),
                    prefixIcon: Icon(Icons.add_circle_outline, color: kAccentColor, size: 20.sp),
                    suffixIcon: IconButton(icon: const Icon(Icons.check, color: kAccentColor), onPressed: () => _addDisease(_inputCtrl.text)),
                    filled: true,
